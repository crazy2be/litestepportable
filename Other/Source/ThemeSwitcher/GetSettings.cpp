...
void SetValue(string Setting, string Value) {
    ...
    try {
        ...
        if (SetRead.is_open()) {
            ...
            SetRead.close();
            ofstream SetWrite ("Settings.ini", ios::trunc);
            if (SetWrite.is_open()) {
                SetWrite << ToWrite;
            } else {
                 Error("Unable to open Settings.ini for writing!");
            }
            SetWrite.close();
        } else {
            Error("Unable to open Settings.ini for reading!");
        ...
        Error("Error occured while reading or writing file...");
    }
}
